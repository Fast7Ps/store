...
function initFlashSales() {
  ...
  scroll.innerHTML = flashProducts.map(p => {
    ...
    return `<div class="product-card" onclick="openDetail(${p.id})" style="flex:0 0 160px; margin-bottom:0">
      ...
      <div class="product-price" style="font-size:0.8rem; padding:0 6px">${CURRENCY}${typeof getCardPrice === 'function' ? getCardPrice(p) : p.price}</div>
...
function initWaChatWidget(enabled, greeting, waNumber) {
  ...
  const linkEl = document.getElementById('waChatDirectLink') || document.getElementById('waChatLink');
  if (linkEl && cleanNum) {
