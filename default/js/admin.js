...
function updateStats() {
  try {
    ...
    var wholesaleCustomers = (function() {
      try {
    ...
    var statWholesaleCustomers = document.getElementById('statWholesaleCustomers');
    if (statProducts) statProducts.textContent = products.length;
    if (statOrders) statOrders.textContent = orders.length;
    
    let currencySymbol = '₪';
    try {
      if (typeof settings !== 'undefined' && settings.currency) {
        currencySymbol = settings.currency;
      } else if (typeof adminSettings !== 'undefined' && adminSettings.currency) {
        currencySymbol = adminSettings.currency;
      }
    } catch(e) {}
    
    if (statRevenue) statRevenue.textContent = currencySymbol + totalRevenue.toFixed(2);
    if (statCustomers) statCustomers.textContent = customers.size;
    if (statWholesaleCustomers) statWholesaleCustomers.textContent = wholesaleCustomers.size;
    ...
    try {
        ...
        if (o && o.status !== 'canceled') {
          (o.items || []).forEach(item => {
            if (item && item.id) {
              ...
              if (!productSales[id]) {
                productSales[id] = { id, name, qty: 0 };
              }
              productSales[id].qty += parseInt(item.qty || 1, 10);
            }
          });
        }
      });
      ...
      if (topProductsList) {
        if (topProducts.length === 0) {
          topProductsList.innerHTML = '<div style="color:var(--text-muted);font-size:0.8rem;text-align:center;padding:10px 0">لا توجد بيانات مبيعات بعد</div>';
        } else {
          topProductsList.innerHTML = topProducts.map((p, idx) => {
            return `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:${idx === topProducts.length - 1 ? 'none' : '1px solid var(--border)'}">
                <div style="display:flex;align-items:center;gap:10px;max-width:80%">
                  ...
                  <span style="font-weight:700;font-size:0.82rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${p.name}</span>
                </div>
                <span style="font-weight:800;color:var(--accent);font-size:0.82rem">${p.qty} قطعة</span>
              </div>
            `;
          }).join('');
        }
    ...
    try {
      ...
      if (topCitiesList) {
        if (topCities.length === 0) {
          topCitiesList.innerHTML = '<div style="color:var(--text-muted);font-size:0.8rem;text-align:center;padding:10px 0">لا توجد بيانات مدن بعد</div>';
        } else {
          topCitiesList.innerHTML = topCities.map((c, idx) => {
            return `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:${idx === topCities.length - 1 ? 'none' : '1px solid var(--border)'}">
                <div style="display:flex;align-items:center;gap:10px">
                  <span style="font-weight:900;color:var(--text-muted);font-size:0.85rem">#${idx + 1}</span>
                  <span style="font-weight:700;font-size:0.82rem">${c.city}</span>
                </div>
                <span style="font-weight:800;color:var(--accent);font-size:0.82rem">${c.count} طلب</span>
              </div>
            `;
          }).join('');
        }
      ...
      console.error('Error calculating top cities:', e);
    }
    ...
    try {
      ...
      const topActiveDaysList = document.getElementById('topActiveDaysList');
      if (topActiveDaysList) {
        if (topDays.length === 0) {
          topActiveDaysList.innerHTML = '<div style="color:var(--text-muted);font-size:0.8rem;text-align:center;padding:10px 0">لا توجد بيانات نشاط بعد</div>';
        } else {
          topActiveDaysList.innerHTML = topDays.map((d, idx) => {
            return `
              <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:${idx === topDays.length - 1 ? 'none' : '1px solid var(--border)'}">
                <div style="display:flex;align-items:center;gap:10px">
                  <span style="font-weight:900;color:var(--text-muted);font-size:0.85rem">#${idx + 1}</span>
                  <span style="font-weight:700;font-size:0.82rem">${d.day}</span>
                </div>
                <span style="font-weight:800;color:var(--accent);font-size:0.82rem">${d.count} طلب</span>
              </div>
            `;
          }).join('');
        }
      ...
      console.error('Error calculating active days:', e);
    }
  } catch (err) {
    console.error('General error in updateStats:', err);
  }
}
