...
function applyLanguage(){
  ...
  if(btn){
    ...
    btn.title=l==='ar'?__('lang_switch'):__('lang_switch')
  }
  document.querySelectorAll('[data-i18n]').forEach(el=>{
    ...
    if(k) {
      ...
      if (icon) {
        el.innerHTML = '';
        el.appendChild(icon);
        el.appendChild(document.createTextNode(' ' + __(k)));
      } else {
        el.textContent=__(k);
      }
    }
...
function renderCustomSections() {
  ...
  sections.forEach(function(sec, idx) {
    ...
    if (sec.type === 'hero') {
    ...
    } else if (sec.type === 'breaking') {
      var bkText = sec.text || sec.title || '';
      var bkBg = sec.bgColor || '#1a5579';
      var bkBorder = sec.borderColor || sec.bgColor || '#1a5579';
      var bkFg = sec.textColor || '#ffffff';
      var bkLink = String(sec.link || '').trim();
      ...
      var bkBoxStyle = 'text-align:center;background-color:' + bkBg + ';border:4px solid ' + bkBorder + ';box-shadow:' + bkShadow + ';border-radius:12px;padding:12px 16px;color:' + bkFg + ';';
      if (bkDeco === 'wave') {
      ...
      var bkInner = bkAnim === 'marquee' ? '<span class="pb-bn-mq" style="color:inherit;font-weight:700">' + bkText + '</span>' : '<span style="color:inherit;font-weight:700">' + bkText + '</span>';
      var bkBox = '<div class="breaking-text' + bkAnimClass + bkDecoClass + '" style="' + bkBoxStyle + '">' + bkInner + '</div>';
      var bkOuter = '<div class="breaking">' + (bkLink ? '<a href="' + bkLink + '" style="text-decoration:none;display:block">' + bkBox + '</a>' : bkBox) + '</div>';
      el.style.cssText = 'padding:16px 16px;margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;';
      el.innerHTML = bkOuter;
    } else if (sec.type === 'note' || sec.type === 'alert') {
      var ntIcon = sec.icon || (sec.type === 'alert' ? '⚠️' : '📝');
      var ntBg = sec.bgColor || (sec.type === 'alert' ? '#fef3c7' : '#eff6ff');
      var ntAcc = sec.accentColor || (sec.type === 'alert' ? '#f59e0b' : '#3b82f6');
      var ntFg = sec.textColor || (sec.type === 'alert' ? '#92400e' : '#1e293b');
      el.style.cssText = 'padding:16px 16px;margin:0 auto;max-width:1200px;width:100%;box-sizing:border-box;';
      el.innerHTML = '<div style="background:'+ntBg+';border-right:4px solid '+ntAcc+';border-radius:12px;padding:16px 18px;display:flex;gap:12px;align-items:flex-start;box-shadow:0 2px 8px rgba(0,0,0,.05)">'
        + (ntIcon ? '<span style="flex-shrink:0;width:34px;height:34px;border-radius:10px;background:'+ntAcc+';color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:1rem">'+ntIcon+'</span>' : '')
        + '<div style="flex:1;min-width:0">'
        + (sec.title ? '<div style="font-size:.95rem;font-weight:800;color:'+ntFg+';margin-bottom:4px">'+sec.title+'</div>' : '')
        + (sec.text ? '<div style="font-size:.85rem;line-height:1.8;color:'+ntFg+';opacity:.95">'+sec.text+'</div>' : '')
        + '</div></div>';
...
function reorderHomeSections() {
  ...
  const custSecs = mktData.customSections || [];
  const elements = [];
  order.forEach(key => {
    if (key.startsWith('_custom_')) {
      var ci = custSecs.findIndex(function(s) { return s._id === key; });
      var cel = ci >= 0 ? document.getElementById('pbCustom_' + ci) : null;
...
function applyMarketing() {
  ...
  const showHeaderSoc = soc.showInHeader !== false;

  const fbBtn = document.getElementById('fbBtn');
  if (fbBtn) {
    if (showHeaderSoc && soc.facebook) { fbBtn.href = soc.facebook; fbBtn.style.display = ''; }
    else { fbBtn.style.display = 'none'; }
  }
  const waBtn = document.getElementById('waBtn');
  if (waBtn) {
    if (showHeaderSoc && soc.whatsapp) { waBtn.href = soc.whatsapp; waBtn.style.display = ''; }
    else { waBtn.style.display = 'none'; }
  }
  const igBtn = document.getElementById('igBtn');
  if (igBtn) {
    if (showHeaderSoc && soc.instagram) { igBtn.href = soc.instagram; igBtn.style.display = ''; }
    else { igBtn.style.display = 'none'; }
  }
  const ttBtn = document.getElementById('ttBtn');
  if (ttBtn) {
    if (showHeaderSoc && soc.tiktok) { ttBtn.href = soc.tiktok; ttBtn.style.display = ''; }
    else { ttBtn.style.display = 'none'; }
  }
  const xBtn = document.getElementById('xBtn');
  if (xBtn) {
    if (showHeaderSoc && soc.twitter) { xBtn.href = soc.twitter; xBtn.style.display = ''; }
...
function cardSelectOption(id, btn) {
  ...
  if (v && parseFloat(v.price) > 0) {
    const priceEl = card.querySelector('.price, .feat-price, .flash-card-prices');
    if (priceEl) {
      let basePrice = parseFloat(v.price);
      if (isWholesale) {
        const vwp = parseFloat(v.wholesalePrice);
        basePrice = vwp > 0 ? vwp : Math.round(basePrice * (1 - ((adminSettings && adminSettings.wholesaleDiscount) || 15) / 100));
      }
      const offerPrice = calcOfferPrice(p);
      let finalPrice = basePrice;
      const prodOffer = getProductOffer(p);
      if (offerPrice !== null && prodOffer) {
        if (prodOffer.type === 'percent') finalPrice = Math.round(basePrice * (1 - prodOffer.value / 100));
        else if (prodOffer.type === 'fixed') finalPrice = Math.max(0, basePrice - prodOffer.value);
      }
      let html = `${CURRENCY}${finalPrice}`;
      let oldPrice = parseFloat(v.oldPrice) || p.oldPrice;
      if (offerPrice !== null) {
        html += ` <span class="old-price">${CURRENCY}${basePrice}</span>`;
      } else if (oldPrice && oldPrice > finalPrice) {
        html += ` <span class="old-price">${CURRENCY}${oldPrice}</span>`;
      }
      priceEl.innerHTML = html + wBadge();
    }
  }
}
...
function toggleSearch() {
  ...
  if (!backdrop) {
    ...
    backdrop.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:290;backdrop-filter:blur(6px);display:none;';
  ...
  if (bar.style.display === 'none' || !bar.style.display) {
    ...
    bar.style.animation = 'searchDropDown .25s ease';
...
function openDetail(id, fromRoute) {
  ...
  if (addBtn) {
    if (p.stock === 0) {
      addBtn.innerHTML = '<i class="fa-solid fa-ban"></i> نفذ';
      addBtn.disabled = true;
      addBtn.style.opacity = '.5';
      addBtn.style.cursor = 'not-allowed';
      addBtn.style.background = '#94a3b8';
    } else {
      addBtn.innerHTML = `<i class="fa-solid fa-cart-plus"></i> ${__('addToCartShort')}`;
...
const discVal = marketingData.fbt?.discount || 10;
  ...
  if (p.options && p.options.length) {
    ...
    updateDetailThumbs(p);
    setTimeout(() => {
      const firstBtn = variantsEl.querySelector('.variant-btn');
  ...
  if (!fromRoute) {
    const isHome = document.getElementById('homePage') && document.getElementById('homePage').classList.contains('active');
    const isCats = document.getElementById('categoriesPage') && document.getElementById('categoriesPage').classList.contains('active');
    if (isHome || isCats) {
      const searchInput = document.getElementById('searchInput');
      window._homeReturnState = {
        productId: id,
        cat: currentCat,
        brand: currentBrand,
        ...
        fromPage: isHome ? 'homePage' : 'categoriesPage'
      };
    }
  }
  showPage('detailPage');
...
function selectVariant(idx, el) {
  ...
  if (currentVariant) {
    let basePrice = parseFloat(currentVariant.price) || currentProduct.price || 0;
    if (isWholesale) {
      const vwp = parseFloat(currentVariant.wholesalePrice);
      basePrice = vwp > 0 ? vwp : Math.round(basePrice * (1 - ((adminSettings && adminSettings.wholesaleDiscount) || 15) / 100));
    }
    const offerPrice = calcOfferPrice(currentProduct);
    let finalPrice = basePrice;
    const prodOffer = getProductOffer(currentProduct);
    if (offerPrice !== null) {
      if (prodOffer) {
        if (prodOffer.type === 'percent') finalPrice = Math.round(basePrice * (1 - prodOffer.value / 100));
        else if (prodOffer.type === 'fixed') finalPrice = Math.max(0, basePrice - prodOffer.value);
    ...
    let html = `<span style="font-size:1.5rem;font-weight:900;color:var(--accent)">${CURRENCY}${finalPrice}${wBadge()}</span>`;
    let oldPrice = parseFloat(currentVariant.oldPrice) || currentProduct.oldPrice;
    if (offerPrice !== null) {
      html += `<span style="font-size:.95rem;text-decoration:line-through;color:#94a3b8;font-weight:500;margin-right:8px">${CURRENCY}${basePrice}</span>`;
      if (prodOffer) {
        html += `<span style="font-size:.7rem;font-weight:800;background:#f59e0b;color:#fff;padding:3px 10px;border-radius:999px;line-height:1;margin-right:4px;display:inline-block"><i class="fa-solid fa-gift"></i> ${prodOffer.badge||prodOffer.name}</span>`;
      }
    } else if (oldPrice && oldPrice > finalPrice) {
      html += `<span style="font-size:.95rem;text-decoration:line-through;color:#94a3b8;font-weight:500;margin-right:8px">${CURRENCY}${oldPrice}</span>`;
      const discountPercent = Math.round((oldPrice - finalPrice) / oldPrice * 100);
      if (discountPercent > 0) {
        html += `<span class="discount-badge" style="position:static;display:inline-block;margin-right:6px;font-size:.7rem">-${discountPercent}%</span>`;
      }
    }
    document.getElementById('detailPrice').innerHTML = html;
  }
...
function populateZones() {
  ...
  if (!sel) return;
  const zones = loadDeliveryZones();
  if (typeof isWholesale !== 'undefined' && isWholesale) {
    zones.push({ name: 'استلام داخل المحل', price: 0 });
  }
  ...
  grid.innerHTML = zones.map(z => {
    const isPickup = z.name === 'استلام داخل المحل';
    const iconClass = isPickup ? 'fa-solid fa-store' : 'fa-solid fa-truck';
    return `
      <div class="zone-card ${sel.value === z.name ? 'selected' : ''}" onclick="selectZoneCard('${z.name}', this)">
        <div class="zone-card-icon"><i class="${iconClass}"></i></div>
...
function getCardPrice(p) {
  ...
  if (price === 0) {
    if (p.options && p.options.length) {
      ...
      const firstVal = firstOpt && firstOpt.values && firstOpt.values[0];
      if (firstVal && parseFloat(firstVal.price) > 0) {
        price = parseFloat(firstVal.price);
      }
    } else if (p.variants && p.variants.length) {
      const firstVar = p.variants[0];
      if (firstVar && parseFloat(firstVar.price) > 0) {
        price = parseFloat(firstVar.price);
      }
    }
  }
  return price;
...
function wholesaleOf(p) {
  ...
  const baseP = getCardPrice(p);
  return Math.round(baseP * (1 - disc / 100));
}

function wPrice(p) {
  if (isWholesale) return wholesaleOf(p);
  return p ? (getCardPrice(p) || p.price || 0) : 0;
}
...
function goBack() {
  ...
  if (window.history.length > 1 && window._homeReturnState) {
    history.back();
  } else {
    location.hash = '#home';
  }
...
function handleRoute() {
  ...
  const isLeavingDetail = !hash.startsWith('#product/') && !hash.startsWith('#order/');
  ...
  if (isLeavingDetail && window._homeReturnState) {
    const ret = window._homeReturnState;
    window._homeReturnState = null;

    // Restore search query if it was active
    const searchInput = document.getElementById('searchInput');
    if (searchInput && ret.searchQuery) {
      searchInput.value = ret.searchQuery;
      const searchBar = document.getElementById('searchBar');
      if (searchBar) searchBar.style.display = 'block';
      renderProducts(products.filter(p => p.name.toLowerCase().includes(ret.searchQuery.toLowerCase())));
    }

    // Restore scroll position
    if (typeof ret.scrollY === 'number' && ret.scrollY > 0) {
      const maxAttempts = 50;
      let attempts = 0;
      while (
        document.getElementById('productsGrid') &&
        document.getElementById('productsGrid').children.length < _allFilteredProducts.length &&
        document.documentElement.scrollHeight < ret.scrollY + window.innerHeight &&
    ...
    // Highlight the product card that the user came from
    if (ret.productId) {
      if (!document.getElementById('highlight-style')) {
        const sheet = document.createElement('style');
        sheet.id = 'highlight-style';
        sheet.innerHTML = `
          @keyframes highlightFlash {
            0% { transform: scale(1); box-shadow: 0 0 0 0 rgba(245, 158, 11, 0.7); border-color: #f59e0b; background: rgba(245, 158, 11, 0.1); }
            50% { transform: scale(1.02); box-shadow: 0 0 15px 5px rgba(245, 158, 11, 0.4); border-color: #f59e0b; background: rgba(245, 158, 11, 0.15); }
      ...
      setTimeout(function() {
        const card = document.querySelector(`.product-card[data-id="${ret.productId}"], .mini-card[data-id="${ret.productId}"], .flash-card[data-id="${ret.productId}"]`);
        if (card) {
...
function adminSaveMarketing(subTab = 'seo') {
  ...
  if (subTab === 'seo') {
    ...
    var showInHeaderEl = document.getElementById('admMktSocialShowInHeader');
    currentData.social = {
      ...
      showInHeader: showInHeaderEl ? showInHeaderEl.checked : true
    };
...
function quickWaOrder() {
  ...
  if (p.options && p.options.length) {
    ...
    if (!currentVariant) {
      showToast("يرجى اختيار خيار من المتغيرات", "error");
      if (typeof highlightVariantSelector === 'function') highlightVariantSelector();
      return;
    }
  }
...
function initSmartSearch() {
  ...
  function updateClear() {
    if (bar) bar.classList.toggle('has-text', !!input.value.trim());
  }
  ...
  if (closeBtn) closeBtn.addEventListener('click', closeSearchPopup);
...
function verifyFeePayCode(code, amt) {
  ...
  log.push({ action: 'payment', plan:'free', amount:amt, code:code, details:'تسديد رسوم متراكمة بقيمة ' + amt + ' ₪ (بواسطة الكود: ' + code + ')', date:new Date().toLocaleString('ar-SA') });
...
function checkSuspension() {
  // التحويل بين index وmaintenance يتم حصرياً في panel-link.js (مصدر الحقيقة:
  // لوحة التحكم). هنا فقط نمسح أي علامة قديمة عالقة حتى لا تسبب حلقة ريفرش
  // بين index وmaintenance، بينما panel-link يقرر التحويل من الخادم مباشرة.
  try { localStorage.removeItem('mycart_store_suspended'); } catch (e) {}
}

document.addEventListener('DOMContentLoaded', function(){ renderFeeAlert(); updateNotifBadge(); updateAdminNotifBadge(); checkSuspension(); applyHeaderDecoration(); refreshLoginNavItem(); });
...
// ===== Supabase cloud data refresh =====
function refreshFromCloud() {
  if (window.__supabaseRefreshed) return;
  ...
  try {
    ...
    if (typeof applyStoreCardVisibility === 'function') applyStoreCardVisibility();
    if (typeof applyLanguage === 'function') applyLanguage();
    if (typeof applySectionOverrides === 'function') applySectionOverrides();
    if (typeof initFlashSales === 'function') initFlashSales();
    if (typeof reorderHomeSections === 'function') reorderHomeSections();
    if (typeof applyMarketing === 'function') applyMarketing();
    if (typeof applyHeaderDecoration === 'function') applyHeaderDecoration();
    if (typeof renderCategories === 'function') renderCategories();
    if (typeof renderProducts === 'function') renderProducts(typeof getFilteredProducts === 'function' ? getFilteredProducts() : products);
    applyAccentColor(adminSettings.accentColor || '#ef4444');
    const nEl = document.getElementById('storeName');
    if (nEl) nEl.textContent = adminSettings.storeName || 'متجري';
    const tEl = document.getElementById('storeTagline');
    if (tEl) tEl.textContent = adminSettings.tagline || '';
    document.title = adminSettings.storeName || 'متجري';
  } catch (e) {
    console.warn('Cloud refresh error:', e);
  }
}
window.addEventListener('supabase:data-loaded', refreshFromCloud);
if (window.__supabasePreloaded) refreshFromCloud();
